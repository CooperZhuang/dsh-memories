...
**被输出上限截断的窗口不推进水位线，而是退避重试**：一次 `max-tokens` 截断（一个可用记忆都没写出）
不算「抽过」，那段转写原样留着等下轮再读——旧行为会把它标成已处理，于是内容静默丢失。保住内容的代价
是重复调用，而这个失败**基本是确定性的**（转写比 `extractMaxOutputTokens` 能描述的更大，或继承来的
推理等级把思考也算进同一个天花板），照周期重试等于每 `extractIntervalMinutes`「订阅」一次满额调用。
所以连续撞上限按 30m → 1h → 2h → 4h → 6h 退避（`incomplete` 不退避，那是没收尾的流，下轮有机会成功）：

- 冷却期内的会话在周期轮次里直接跳过（行里计成 `capped`），且**不占** `maxSessionsPerPass` 名额；
- `/memories mine` 照旧立刻执行——你已经决定花这笔钱；
- **改动日志点名的那两个旋钮（`extractMaxOutputTokens`、抽取路由）会立刻作废冷却**，不会出现「按提示
  调了却还要等几小时」；任何一次完成的抽取也会清掉记录。
...
| `extractMaxOutputTokens` | `8192` | 抽取调用的输出上限（2026-09-23 由 2048 提高、2026-09-24 由 4096 提到 8192：中文写满 5 条记忆加摘要会超过 2048，实测第一次 `/memories mine` 就被截断。上限是天花板不是花费，提前结束的回复不花这部分；真被截断时先抢救已写完的条目，一个都没写出来才报 `max-tokens` 并按上面那条退避，日志里会点名该调这个旋钮或换路由） |
...
| 撞上限的窗口不再静默丢弃，改为退避重试（2026-09-24，效果与成本的平衡） | ✅ 单测：①撞 `max-tokens` 的窗口不推进水位线（内容不丢），完成的两类（`memories`、`empty-reply`）才推进；②冷却期内的后台轮次跳过并把理由计成 `capped`，**且不占** `maxSessionsPerPass` 名额（两会话 + 上限 1：冷却的那个让位，另一个照抽，行里不出现 `pass-cap`）；③`/memories mine` 仍立刻执行（调用者已决定花钱）；④`extractMaxOutputTokens` 或路由一改，冷却立刻失效（日志点名要调的旋钮不能变成「按提示改了却还要等几小时」）；⑤完成的抽取清掉记录，下个窗口按正常节奏抽。等待阶梯 30m→1h→2h→4h→6h 由纯函数单测钉住（不会短于用户配置的 `extractIntervalMinutes`，且有上限）。针对闸门、冷却生效、冷却作废三处各做过一次变异测试确认用例真会失败。现场依据：真实日志里 5 次撞上限全是 `out` 顶满 4096 的满额调用，其中 `session-11111111` 04:38 撞 `24490 in / 4096 out`、05:08 只抽到了更新的 `5652` 窗口——按旧代码那段 24490 的转写再也不会被读到 |
