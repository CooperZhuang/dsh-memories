...
4. **分层注入（上下文）** — 一次会话只注入**一次**有界摘要：在第一个「有话说」的 step 进入，
   ...
   之后**不再重注入**——会话中途写入的记忆不刷新摘要，要用就 `memory_search`；`/compact` 或
   清空会话后会自动重新注入（那时整段对话已经换掉），所以记忆不会被压缩吃掉。代价是模型手里
   的摘要可能比记忆库旧一点，收益是一段 50 轮的会话只为记忆付一次约 2～4KB，而不是每轮一次。
...
| 注入节奏（一次会话恰好一次） | ✅ 真机进程：带插件的 headless 组合跑真实会话，注入恰好 1 次（3943 字节） |
